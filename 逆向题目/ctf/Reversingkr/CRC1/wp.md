md...
[wp](https://github.com/DoubleLabyrinth/reversing.kr/tree/master/CRC1)

就逆向角度没有任何理解难度 难点在于怎么逆算法...

首先 题目那奇奇怪怪的算法其实是标准的CRC64
所以可以抽象出来(函数模块化)

然后用`Meet-In-Middle`的思想
针对8个字符分为0~3 4~7 分别计算CRC

其实当时在车上也想到这个思路了 但对CRC完全不熟悉 写不出来...
这里直接搬exp了罢
```c
#include <stddef.h>
#include <stdint.h>
...
然后直接开O3优化
`gcc -O3 .\find_flag.c -o find_flag.exe`
